        ...
        with open(filename, 'rb') as fh:
            for key, value in sockList.items():
                ...
                try:
                    ...
                    value.send((data2send + '-' * (100 - len(
                        data2send))).encode())  # encrypt_dat1 MACHI HERE YOU ARE SENDING LEN+8 AS LENGTH CORRECT AGI NODU
                            ...
                            try:
                                while status == '':
                                    ...
                                    print("Sending file part " + str(parts2send[0]) + " : " + \
                                          str(100 - int(
                                              (BytesSent / int(OffsetLen[(parts2send[0] - 1)]) * 100))) + "%\r", end='')
                                Recv_ACK = True
                            except timeout:
                                value.send(encrypt_dat11)
                                time.sleep(0.01)
